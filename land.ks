...
if hastarget {
    ...
    else {set landingsite to latlng(target:geoposition:lat-ship:geoposition:lat, target:geoposition:lng-ship:geoposition:lng).
    print "No target chosen, soft water landing at impact position.".
    }
...
function hv { 
    ...
    IF C:SENDMESSAGE(MESSAGE) {PRINT "Catching".}
    }
